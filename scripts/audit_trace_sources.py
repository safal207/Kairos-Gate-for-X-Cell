#!/usr/bin/env python3
"""Retrieve TRACE evidence sources and emit SHA-256 audit metadata.

The script does not modify the manifest, redistribute source content, or claim
independent reproduction. It records retrieval evidence only.
"""
...
EXPECTED_SCHEMA = "kairos.trace-archaic-source-manifest.v0.1"
...
def digest_url(url: str, timeout: float) -> dict[str, Any]:
    ...
    with tempfile.NamedTemporaryFile(prefix="kairos-trace-", delete=True) as tmp:
        with urlopen(request, timeout=timeout) as response:  # noqa: S310 - manifest-pinned audit URLs
...
def main() -> int:
    ...
    output = {
        "schema": "kairos.trace-archaic-source-digests.v0.1",
        "case_id": manifest["case_id"],
        "manifest_schema": manifest["schema"],
        "authority": {
            "classification": "RESEARCH_ONLY",
            "independent_reproduction_claim_authorized": False,
