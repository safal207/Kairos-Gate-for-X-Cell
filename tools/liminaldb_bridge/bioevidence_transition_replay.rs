...
fn main() -> Result<()> {
    ...
    if bundle.schema_version != "0.1.0"
        || bundle.bridge_profile != "org.kairos-gate.liminaldb-bioevidence-bridge.v0.1"
    ...
    {
        return Err(anyhow!("storage/authority boundary violation"));
    }
    if bundle.records.len() != 6 {
        return Err(anyhow!("expected exactly six transition records"));
    }
    ...
    let receipt = Receipt {
        ...
        artifact_type: "liminaldb_bioevidence_replay_receipt",
